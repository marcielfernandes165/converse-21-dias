 ...
 */
export const dayProgress = mysqlTable("dayProgress", {
  id: int("id").autoincrement().primaryKey(),
...
export const checkpoints = mysqlTable("checkpoints", {
  id: int("id").autoincrement().primaryKey(),
  sessionId: int("sessionId").notNull(),
  dayNumber: int("dayNumber").notNull(), // 8, 15 ou 21
  ...
  feeling: mysqlEnum("feeling", ["more_confident", "equal", "confused"]), // "Você se sente:"
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
